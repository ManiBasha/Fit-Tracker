...
function renderHeader(){
  ...
  const displayName = cache.profile?.name || currentUser?.displayName || '';
  document.getElementById('greeting').textContent = greeting + (displayName ? ', ' + displayName.split(' ')[0] : '') + ' 👋';
  ...
  const initial = (displayName || currentUser?.email || 'U')[0].toUpperCase();
  document.getElementById('avatarInitial').textContent = initial;
  document.getElementById('avatarLg').textContent = initial;
  document.getElementById('profileName').textContent = displayName || 'You';
...
function renderDashboard(){
  ...
  const todaysCalories = caloriesForDate(todayStr(), cache.metricsToday);
  document.getElementById('caloriesVal').textContent = todaysCalories ? todaysCalories.toLocaleString() : '–';
...
function renderRecentWorkouts(){
  ...
  recent.forEach(w => {
    ...
    row.className = 'timeline-item clickable';
    const hits = w.exercises ? w.exercises.filter(e=>e.hit).length : null;
    row.innerHTML = `<div><div class="t-name">${escapeHtml(w.workoutName)}</div><div class="t-date">${niceDate(w.date)}</div></div><span class="pill">${hits!=null ? hits+'/'+w.exercises.length : '✓'}</span>`;
    row.addEventListener('click', () => showWorkoutDetailModal(w));
...
async function renderActivity(range){
  ...
  drawBarChart('calChart', labels, dates.map(d=>caloriesForDate(d, metricsMap[d])), '#FF7A59');
...
function latestWeight(){
  const sorted = [...cache.bodyLog].sort((a,b)=>a.date.localeCompare(b.date));
  return sorted.length ? Number(sorted[sorted.length-1].weight) : null;
}

// Sleep duration from start/end clock time, handling overnight wrap (e.g. 23:00 -> 07:00).
...
// MET-based estimate for resistance/strength training: kcal = MET * 3.5 * weight(kg) / 200 * minutes.
function estimateCalories(durationMin, weightKg, met=6){
  if(!durationMin || !weightKg) return 0;
  return Math.round(met * 3.5 * weightKg / 200 * durationMin);
}

// Combines any wearable-imported daily calorie total with calories computed from logged workouts that day.
function caloriesForDate(dateStr, metricsRecord){
  const wearable = Number(metricsRecord?.calories || 0);
  const workouts = cache.workoutLog.filter(l => l.date === dateStr).reduce((s,l) => s + (l.calories||0), 0);
  return wearable + workouts;
}
...
function openBodyLogModal(dateStr){
  openModal('Log weight', [
    { key:'date', label:'Date', type:'date', value: dateStr || todayStr() },
    { key:'weight', label:'Weight (kg)', type:'number' }
  ], async values => {
    cache.bodyLog.push({ id:'b'+Date.now(), date: values.date, weight: values.weight });
    await store.setData('bodyLog', { entries: cache.bodyLog });
    renderBodyLog();
    renderWeeklyWeightBanner();
  });
}

document.getElementById('addBodyLogBtn').addEventListener('click', () => openBodyLogModal(todayStr()));

function renderWeeklyWeightBanner(){
  const banner = document.getElementById('weeklyWeightBanner');
  const sorted = [...cache.bodyLog].sort((a,b)=>a.date.localeCompare(b.date));
  const latest = sorted[sorted.length-1];
  const weekAgo = addDays(todayStr(), -7);
  const overdue = !latest || latest.date < weekAgo;
  if(overdue){
    banner.style.display = 'flex';
    document.getElementById('weeklyWeightBannerText').textContent = latest
      ? `Last logged ${niceDate(latest.date)}. Log a fresh weigh-in to keep BMI accurate.`
      : `No weight logged yet — add your starting weight to begin tracking BMI.`;
    document.getElementById('weeklyWeightBannerBtn').onclick = () => openBodyLogModal(todayStr());
  } else {
    banner.style.display = 'none';
  }
}
...
function openQuickLogModal(dateStr){
  const existing = cache.metricsWeek[dateStr] || (dateStr===todayStr() ? cache.metricsToday : {}) || {};
  openModal('Log steps, heart rate & sleep', [
    { key:'date', label:'Date', type:'date', value: dateStr },
    ...
    { key:'sleepStart', label:'Sleep start time', type:'time', value: existing.sleepStart || '' },
    { key:'sleepEnd', label:'Wake-up time', type:'time', value: existing.sleepEnd || '' }
  ], async values => {
    const targetDate = values.date || dateStr;
    const clean = {};
    if(values.steps !== '') clean.steps = Number(values.steps);
    if(values.heartRate !== '') clean.heartRate = Number(values.heartRate);
    if(values.sleepStart && values.sleepEnd){
      clean.sleepStart = values.sleepStart;
      clean.sleepEnd = values.sleepEnd;
      clean.sleepHours = calcSleepHours(values.sleepStart, values.sleepEnd);
    }
    const priorRec = await store.getMetrics(targetDate) || {};
    if(priorRec.calories) clean.calories = priorRec.calories; // preserve any CSV-imported wearable total
    await store.setMetrics(targetDate, clean);
    if(targetDate === todayStr()) cache.metricsToday = clean;
    cache.metricsWeek[targetDate] = clean;
...
function parseFirstNumber(s){ const m = String(s).match(/\d+/); return m ? Number(m[0]) : null; }
function parseRepsRange(s){
  const nums = String(s).match(/\d+/g) || [];
  if(!nums.length) return {min:null,max:null};
  return { min:Number(nums[0]), max:Number(nums[nums.length-1]) };
}

function buildComparisonHtml(entry){
  if(!entry.exercises) return '<p class="muted small">Legacy entry — no exercise detail recorded.</p>';
  const hits = entry.exercises.filter(e=>e.hit).length;
  let html = `<div class="compare-summary">
    <div class="bmi-chip"><span class="v">${hits}/${entry.exercises.length}</span><span class="l">Exercises hit</span></div>
    <div class="bmi-chip"><span class="v">${entry.durationMin||0} min</span><span class="l">Duration</span></div>
    <div class="bmi-chip"><span class="v">${entry.calories||0}</span><span class="l">Calories</span></div>
  </div>
  <div class="compare-row head"><span>Exercise</span><span>Planned</span><span>Actual</span></div>`;
  entry.exercises.forEach(ex => {
    html += `<div class="compare-row"><span class="name">${escapeHtml(ex.name)}</span><span>${escapeHtml(ex.plannedSets)}×${escapeHtml(ex.plannedReps)}</span><span class="${ex.hit?'hit':'miss'}">${ex.actualSets}×${ex.actualReps}</span></div>`;
  });
  return html;
}
...
function renderTodayWorkoutBlock(){
  ...
  const compareBox = document.getElementById('todayComparison');
  if(assign === 'rest'){ box.innerHTML = '<p class="muted">Rest day — recovery matters too.</p>'; logBtn.style.display='none'; compareBox.innerHTML=''; return; }
  if(assign === 'walk'){ box.innerHTML = '<p class="muted">Walk — 30–60 min.</p>'; logBtn.style.display='none'; compareBox.innerHTML=''; return; }
  const w = plan.workouts[assign];
  if(!w){ box.innerHTML = '<p class="muted">No workout assigned today.</p>'; logBtn.style.display='none'; compareBox.innerHTML=''; return; }
  logBtn.style.display = 'block';
  const todayEntry = cache.workoutLog.find(l => l.date === todayStr());
  logBtn.textContent = todayEntry ? "Re-log today's workout" : 'Log actual workout';
  ...
  compareBox.innerHTML = todayEntry ? buildComparisonHtml(todayEntry) : '';
}

/**
 * Log 2 — the actual workout tracker. Lets the user pick a date and a
 * workout, log the sets/reps they actually did per exercise (compared
 * against the template's plan), and a session duration used to estimate
 * calories burned (MET formula, using the most recent logged body weight).
 */
function openWorkoutLogModal(dateStr){
  const dayKey = DAY_KEYS[new Date(dateStr+'T00:00:00').getDay()];
  const scheduledKey = plan.schedule[dayKey];
  const workoutKeys = Object.keys(plan.workouts);
  if(!workoutKeys.length){ alert('Add a workout template first (Workouts → Templates).'); return; }
  const selectedKey = workoutKeys.includes(scheduledKey) ? scheduledKey : workoutKeys[0];

  const overlay = document.createElement('div');
  overlay.className = 'modal-overlay';
  const modal = document.createElement('div');
  modal.className = 'modal';
  modal.innerHTML = `<h3>Log actual workout</h3>`;

  const dateLabel = document.createElement('label'); dateLabel.textContent = 'Date';
  const dateInput = document.createElement('input'); dateInput.type = 'date'; dateInput.value = dateStr;
  modal.append(dateLabel, dateInput);

  const wLabel = document.createElement('label'); wLabel.textContent = 'Workout';
  const wSelect = document.createElement('select');
  workoutKeys.forEach(k => { const opt=document.createElement('option'); opt.value=k; opt.textContent=plan.workouts[k].name; if(k===selectedKey) opt.selected=true; wSelect.appendChild(opt); });
  modal.append(wLabel, wSelect);

  const durLabel = document.createElement('label'); durLabel.textContent = 'Duration (minutes)';
  const durInput = document.createElement('input'); durInput.type = 'number'; durInput.min = '1'; durInput.value = '30';
  modal.append(durLabel, durInput);

  const rowsHead = document.createElement('div');
  rowsHead.className = 'wlog-cols-head';
  rowsHead.innerHTML = '<span>Exercise (planned)</span><span>Sets</span><span>Reps</span>';
  ...
  function renderExerciseRows(){
    ...
    const w = plan.workouts[wSelect.value];
    if(!w || !w.exercises.length){ rowsContainer.innerHTML = '<p class="muted small">No exercises in this workout — edit it in Templates first.</p>'; return; }
    w.exercises.forEach(ex => {
      const row = document.createElement('div');
      row.className = 'wlog-row';
      const setsGuess = parseFirstNumber(ex.sets) || '';
      const repsGuess = parseRepsRange(ex.reps).max || '';
      const nameCell = document.createElement('span');
      nameCell.className = 'wlog-name';
      nameCell.innerHTML = `${escapeHtml(ex.name)}<span class="wlog-plan">Planned: ${escapeHtml(ex.sets)}×${escapeHtml(ex.reps)}</span>`;
      const setsInput = document.createElement('input'); setsInput.type='number'; setsInput.min='0'; setsInput.value=setsGuess; setsInput.dataset.exId=ex.id; setsInput.dataset.field='sets';
      const repsInput = document.createElement('input'); repsInput.type='number'; repsInput.min='0'; repsInput.value=repsGuess; repsInput.dataset.exId=ex.id; repsInput.dataset.field='reps';
      row.append(nameCell, setsInput, repsInput);
      rowsContainer.appendChild(row);
    });
  }
  renderExerciseRows();
  wSelect.addEventListener('change', renderExerciseRows);
  ...
  save.addEventListener('click', async () => {
    const w = plan.workouts[wSelect.value];
    if(!w){ overlay.remove(); return; }
    const exercises = w.exercises.map(ex => {
      const setsInput = rowsContainer.querySelector(`input[data-ex-id="${ex.id}"][data-field="sets"]`);
      const repsInput = rowsContainer.querySelector(`input[data-ex-id="${ex.id}"][data-field="reps"]`);
      const plannedSets = parseFirstNumber(ex.sets);
      const plannedReps = parseRepsRange(ex.reps);
      const actualSets = setsInput ? Number(setsInput.value)||0 : 0;
      const actualReps = repsInput ? Number(repsInput.value)||0 : 0;
      const hit = plannedSets!=null && actualSets >= plannedSets && (plannedReps.min==null || actualReps >= plannedReps.min);
      return { id: ex.id, name: ex.name, plannedSets: ex.sets, plannedReps: ex.reps, actualSets, actualReps, hit };
    });
    const duration = Number(durInput.value) || 0;
    const weight = latestWeight() || 70; // fallback average if no weight logged yet
    const calories = estimateCalories(duration, weight);
    const targetDate = dateInput.value || dateStr;

    cache.workoutLog = cache.workoutLog.filter(l => l.date !== targetDate); // one logged session per date
    cache.workoutLog.push({
      id:'w'+Date.now(), date: targetDate, workoutKey: wSelect.value, workoutName: w.name,
      durationMin: duration, calories, exercises
    });
    await store.setData('workoutLog', { entries: cache.workoutLog });
    await recomputeStreak(targetDate, true);
...
document.getElementById('logWorkoutBtn').addEventListener('click', () => openWorkoutLogModal(todayStr()));
...
function renderWorkoutHistory(){
  ...
  sorted.forEach(w => {
    ...
    row.className = 'timeline-item clickable';
    const hits = w.exercises ? w.exercises.filter(e=>e.hit).length : null;
    const pillText = hits!=null ? `${hits}/${w.exercises.length}` : '✓';
    row.innerHTML = `<div><div class="t-name">${escapeHtml(w.workoutName)}</div><div class="t-date">${niceDate(w.date)}${w.durationMin ? ' · '+w.durationMin+' min' : ''}</div></div><span class="pill">${pillText}</span>`;
    row.addEventListener('click', () => showWorkoutDetailModal(w));
    list.appendChild(row);
  });
}

function showWorkoutDetailModal(entry){
  const overlay = document.createElement('div');
  overlay.className = 'modal-overlay';
  const modal = document.createElement('div');
  modal.className = 'modal';
  modal.innerHTML = `<h3>${escapeHtml(entry.workoutName)} — ${niceDate(entry.date)}</h3>` + buildComparisonHtml(entry);
  const actions = document.createElement('div');
  actions.className = 'modal-actions';
...
function renderInsights(){
  ...
  setRing(document.getElementById('ringSleep'), avgSleepPct, 34);
  document.getElementById('healthScoreVal').textContent = score;

  document.getElementById('scoreBreakdown').innerHTML = `
    <div class="score-item"><span class="score-dot" style="background:#2DD4A7"></span><span>Workouts</span><strong>${workoutsThisWeek}/${plannedThisWeek}</strong></div>
    <div class="score-item"><span class="score-dot" style="background:#FF6584"></span><span>Steps</span><strong>${Math.round(avgStepsPct*100)}%</strong></div>
    <div class="score-item"><span class="score-dot" style="background:#4C8CFF"></span><span>Sleep</span><strong>${Math.round(avgSleepPct*100)}%</strong></div>
  `;
...
function renderProfile(){
  ...
  document.getElementById('nameInput').value = cache.profile.name || '';
  document.getElementById('ageInput').value = cache.profile.age || '';
  document.getElementById('genderInput').value = cache.profile.gender || 'female';
...
document.getElementById('saveHeightBtn').addEventListener('click', async () => {
  cache.profile.name = document.getElementById('nameInput').value || '';
  cache.profile.age = Number(document.getElementById('ageInput').value) || null;
  cache.profile.gender = document.getElementById('genderInput').value || 'female';
  cache.profile.height = Number(document.getElementById('heightInput').value) || null;
  cache.profile.onboarded = true;
...
/**
 * Shown once, right after a brand-new Google sign-in (profile.onboarded is
 * false). Captures the basics used across the app: BMI (height + weekly
 * weight), and calorie-burn estimates for logged workouts (weight). Age and
 * gender are stored for future calculation features (e.g. BMR/TDEE).
 */
function openOnboardingModal(){
  openModal('Welcome — set up your profile', [
    { key:'name', label:'Name', type:'text', value: currentUser?.displayName?.split(' ')[0] || '' },
    { key:'age', label:'Age', type:'number' },
    { key:'gender', label:'Gender', type:'select', value:'female', options:[
    ...
    await store.setData('profile', cache.profile);
    if(values.weight){
      cache.bodyLog.push({ id:'b'+Date.now(), date: todayStr(), weight: Number(values.weight) });
      await store.setData('bodyLog', { entries: cache.bodyLog });
    }
    renderHeader();
    renderProfile();
